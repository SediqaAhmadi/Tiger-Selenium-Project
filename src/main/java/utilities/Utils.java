...
public class Utils {
    ...
    public static void getBrowser(String browserName, String url) {
        ...
        /*
          ...
          executing our test cases and the reason we have these waits in Selenium is
          because the WebDriver will only handle the browser and will not intract with
          WebElements, we we launch the browser and then close the browser, there are
          many other actions that we will need to perform to execute our tests for
          example click on a button, send a text to a text box and etc... the waits in
          Selenium will help us to handle the WebDriver and make sure it waits for some
          certain condition before moving to next step
         */
    ...
    //this method will help us to use hard wait during test execution if we need to debug our test script 
    ...
    //close() => we use this method to close only one browser that is opended by selenium
    // quit(); => we use this method to close all browsers opened by selenium
    //with selenium, we can open either one or more than one window to execute our test, we will practice with multiple -
    // browsers in the second week of selenum when we get to the windowHundle and windowsHandle. 
    //++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++//
